...
@click.group(
    ...
    \b
    tidydata transform add-column input.csv --column_name 'Added_Column_Data.csv'

    2. Rename a Column:
    \b
    tidydata transform rename-column input.csv --old_name 'Old Name' --new_name 'New Name' --output 'renamed_column_data.csv'
    
    1. View the last `n` rows of the DataFrame:
    \b
    tidydata transform view-tail input.csv --n 10 --output 'viewed_tail_data.csv'
...
@click.option(
    "--value",
    help="The value to be assigned to the new column. Can be a single value or a list/Series with length equal to the DataFrame.",
)
@click.option(
    "--output",
    default=None,
...
def rename_column(input_file, old_name, new_name, output):
    ...
    transformed_data = column_renamed.rename_column(

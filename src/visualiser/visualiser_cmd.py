...
@click.group(
    ...
    \b
    tidydata visualize basic-bar-chart input.csv --x_column 'Category' --output 'bar_chart.png'

    2. Generate a Word Cloud:
    \b
    tidydata visualize wordcloud input.csv --text_column 'Text' --output 'wordcloud.png'
    """
)
def cli():
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Path to save the chart image (optional).",
)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Display percentages instead of counts.",
)
...
@click.option(
    "--title", default="Horizontal Bar Chart", help="Title of the horizontal bar chart."
)
@click.option(
    "--percentage",
    is_flag=True,
...
def horizontal_bar_chart(input_file, x_column, y_column, output, title, percentage):
    ...
    data = pd.read_csv(input_file)
    visualizer = DataVisualizer(data)
    visualizer.horizontal_bar_chart(
        x_column=x_column,
        y_column=y_column,
        percentage=percentage,
        title=title,
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Path to save the word cloud image (optional).",
)
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Path to save the table (optional).",
)
@click.option(
    "--format", default="html", help="Output format for the table (html, png, etc.)."
)
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Path to save the line chart (optional).",
)
...
def line_chart(input_file, x_column, y_column, output, title):
    ...
    visualizer.line_chart(
        x_column=x_column, y_column=y_column, title=title, output_path=output
    )
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Path to save the histogram image (optional).",
)
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Path to save the scatter plot image (optional).",
)

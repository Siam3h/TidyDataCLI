...
@click.group(
    ...
    \b
    tidydata clean handle-missing-values input.csv --method 'fill' --fill_value 'N/A' --output 'filled_data.csv'
    """
)
def cli():
...
@click.option(
    "--date_format",
    default="%Y-%m-%d",
    help="The desired date format (default is %Y-%m-%d).",
)
@click.option(
    "--output",
    default=None,
    type=click.Path(),
    help="Path to save the cleaned data (optional).",
)
def standardize_date(input_file, column, date_format, output):
    ...
    cleaned_data = standardizer.standardize_date(
        column=column, date_format=date_format
    ).data
...
@click.option(
    "--fill_value",
    default=None,
    help='Value to fill missing values with if "fill" method is chosen.',
)
@click.option(
    "--output",
    default=None,
...
def apply_regex_cleaning(input_file, column, pattern, replacement, output):
    ...
    data = pd.read_csv(input_file)
    cleaner = Basic_Cleaner(data)
    cleaned_data = cleaner.apply_regex_cleaning(
...
@click.option(
    "--columns",
    default=None,
    help="Comma-separated list of columns to apply the transformation. If None, all text columns are used.",
)
@click.option(
    "--operation",
    default="lower",
    type=click.Choice(["lower", "upper", "title", "capitalize"]),
    help="Case transformation operation.",
)
@click.option(
    "--output",
    default=None,
    type=click.Path(),
    help="Path to save the transformed data (optional).",
)
